...
class GetGoodById(APIView):
    ...
    def get(self, request, good_id):
        ...
        try:
            ...
            for p in good_pics:
                p_url = '{}{}'.format(base_url, 'pic/{}'.format(p.id))
                response.data['images'][p.id] = p_url
                if p.is_main_pic == 1:
...
# 用于用户发布新商品(GET获取要发布商品使用到的分类信息, POST将发送数据给服务器端进行数据持久化)
    ...
    def get(self, request):
        ...
        try:
            ...
            for c in category_list:
                response.data['categories'][c.id] = c.name
...
# 用于用户对已发布的商品进行查询(GET),修改(PUT),删除(DELETE)
